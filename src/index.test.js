...
const bookings = bookingsData.map((data, index) => Booking.create(index, bookingsData));
...
describe('Room Class Tests', () => {
    ...
    test('should throw an error if check out before check in', () => {
        ...
        const errorMessage = errorMessages.dateRangeInvalid;
...
describe('Booking Class Tests', () => {
    ...
    describe('should return a error if checkOut before checkIn', () => {
        expect(() => bookings[19].datesValidation()).toThrow(errorMessages.dateRangeInvalid);
        expect(() => bookings[1].datesValidation()).toThrow(errorMessages.dateRangeInvalid);
    })
})
